...
export default function PublicChatPage() {
  ...
  useEffect(() => {
    ...
    if (found) {
      setBot(found);
      setMessages([{ role: 'bot', text: `Hello! I am ${found.name}, specialized in ${found.topic}. How can I help you today?` }]);
    }
  }, [id]);
  ...
  if (!bot) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6 text-center">
        <div>
          <h1 className="text-2xl font-headline font-bold mb-2">Bot Not Found</h1>
          <p className="text-muted-foreground">This chatbot link may have expired or is incorrect.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a090c] text-foreground font-body max-w-2xl mx-auto shadow-2xl">
      {/* Header */}
      <header className="p-4 border-b border-primary/10 bg-primary/5 flex items-center justify-between sticky top-0 z-10 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-accent font-medium uppercase tracking-widest">AI Powered</span>
        </div>
      </header>
      ...
      <footer className="p-4 border-t border-border bg-[#0d0c11] pb-8 md:pb-4">
        ...
        <p className="text-[10px] text-center text-muted-foreground mt-3 uppercase tracking-tighter opacity-50">
          Powered by LinkThread Intelligence
        </p>
      </footer>
    </div>
  );
